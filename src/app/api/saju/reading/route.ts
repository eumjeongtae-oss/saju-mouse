...
섹션 ④: 프롬프트에 제공된 [당신의 사주에 있는 특수 기운(살)] 정보를 확인하여, 올해 가장 조심해야 할 달(구체적 N월)에 나의 어떤 '살(도화/역마 등)'이 발동하여 어떤 변화나 충동이 일어날 수 있는지, 그리고 그 시기에 써야 할 현실적인 처세술을 서술 (뚜렷한 살이 없다면 오행의 특징을 기반으로 서술)
extras.bestMonth: 가장 유리한 달 (예: "9월")
extras.worstMonth: 가장 조심할 달 (예: "7월")
extras.cautionPoint: 올해 가장 주의해야 할 구체적인 함정이나 유혹 한 문장
extras.keyAction: 기회를 잡고 위기를 피하기 위한 구체적 행동 지침 한 문장
...
섹션 ②: 프롬프트에 제공된 [당신의 사주에 있는 특수 기운(살)] 정보를 확인하여, 내 사주에 있는 '살' (도화, 역마, 백호 등)을 어떻게 치명적인 무기로 써서 돈으로 연결할 수 있는지 사주적 근거를 들어 3가지 분야 제시 (뚜렷한 살이 없다면 오행의 특징을 기반으로 서술)
...
extras.benefactorZodiac: 금전적으로 유리한 기회를 가져다줄 귀인의 띠와 이유
  ...
  love: `
섹션 ①: 프롬프트에 제공된 [당신의 사주에 있는 특수 기운(살)] 정보를 확인하여, 당신이 연애에서 반복하는 구체적인 패턴을 진단하고, 특히 당신의 '살'(홍염살, 도화살, 화개살 등)이 이성에게 어떤 묘한 매력으로 작용하는지 생생하게 서술 (뚜렷한 살이 없다면 오행의 특징을 기반으로 서술)
...
섹션 ②: 프롬프트에 제공된 [당신의 사주에 있는 특수 기운(살)] 정보와 상대방의 정보를 종합하여, 두 사람의 '살'이 서로 부딪히거나 조화를 이룰 때 어떤 치명적인 스파크나 긍정적 시너지가 발생하는지 생생하게 서술 (뚜렷한 살이 없다면 오행의 특징을 기반으로 서술)
...
function getResponseSchema(theme: FortuneTheme): Schema {
  ...
  if (theme === 'general') {
    ...
    extrasReq.push("birthElement", "careers", "compatibleType", "incompatibleType", "myWeapon", "benefactorZodiac");
    ...
    extrasProps.cautionPoint = { type: SchemaType.STRING };
    extrasProps.myWeapon = { type: SchemaType.STRING };
    extrasProps.benefactorZodiac = { type: SchemaType.STRING };
    extrasReq.push("bestMonth", "worstMonth", "keyAction", "cautionPoint", "myWeapon", "benefactorZodiac");
    ...
    extrasProps.myWeapon = { type: SchemaType.STRING };
    extrasProps.benefactorZodiac = { type: SchemaType.STRING };
    extrasReq.push("compatibilityScore", "coupleType", "synergyPoint", "advice", "myWeapon", "benefactorZodiac");
  }
